...
class StockReceiptDetailsPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<StockReceiptDetailsPage> createState() => _StockReceiptDetailsPageState();
}

class _StockReceiptDetailsPageState extends ConsumerState<StockReceiptDetailsPage> {
    ...
    try {
      ...
      if (result is PostSuccess) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('تم ترحيل أمر التوريد بنجاح'),
  ...
  Future<void> _handleUnpost(StockReceipt receipt) async {
    ...
    try {
      ...
      if (result is UnpostSuccess) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('تم إلغاء ترحيل أمر التوريد بنجاح'),
  ...
  void _handleEdit(StockReceipt receipt) {
    if (receipt.isPosted) {
      showDialog(
        context: context,
        builder: (context) => AlertDialog(
          ...
          content: const Text(
            'لا يمكن تعديل أمر التوريد وهو في حالة (مرحّل).\n\nيرجى إلغاء الترحيل أولاً لتتمكن من التعديل.',
          ),
          actions: [
            TextButton(
    ...
    InventoryRoutes.pushStockReceiptsEdit(context, receipt.id);
  }

  Future<void> _handleDelete(StockReceipt receipt) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('تأكيد الحذف'),
        content: Text('هل أنت تأكد من حذف أمر التوريد ${receipt.receiptNumber}؟'),
        actions: [
          TextButton(
  ...
  Widget build(BuildContext context) {
    final receiptAsync = ref.watch(stockReceiptByIdProvider(widget.receiptId));
      ...
      data: (receipt) {
        if (receipt == null) {
          return Scaffold(
            ...
            body: const Center(child: Text('لم يتم العثور على أمر التوريد')),
          );
        }
        ...
        final totalCostSum = receipt.lines.fold(0.0, (sum, line) => sum + line.totalCost);
        final supplierName = (receipt.supplier != null && receipt.supplier!.trim().isNotEmpty)
            ? receipt.supplier!.trim()
            : 'غير محدد';
        final warehouseName = (receipt.warehouse != null && receipt.warehouse!.trim().isNotEmpty)
        ...
        return Scaffold(
          backgroundColor: scheme.surfaceContainerLowest,
          appBar: CustomAppBar(
            title: formatSaleNumberPrimary(receipt.receiptNumber),
            showBackButton: true,
            actions: [
              CustomAppBarAction(
                icon: receipt.isPosted ? Icons.lock_outlined : Icons.edit_outlined,
                tooltip: receipt.isPosted ? 'تعديل (مرحّل)' : 'تعديل',
                onPressed: _isProcessing ? null : () => _handleEdit(receipt),
              ),
          ...
          bottomNavigationBar: Material(
            ...
            child: SafeArea(
              child: Padding(
                ...
                child: receipt.isDraft
                    ? FilledButton.icon(
                        onPressed: _isProcessing ? null : () => _handlePost(receipt),
                        icon: const Icon(Icons.check_circle_outline),
                        label: const Text('ترحيل أمر التوريد'),
                    ...
                    : OutlinedButton.icon(
                        onPressed: _isProcessing ? null : () => _handleUnpost(receipt),
                        icon: const Icon(Icons.undo),
                        label: const Text('إلغاء ترحيل أمر التوريد'),
