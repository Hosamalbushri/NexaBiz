...
void main() {
  ...
  group('Phase 3 — Authentication, Offline Identity & Authorization Hardening Security Tests', () {
    ...
    test('2. Offline login with invalid password does not load snapshot (repository validation)', () async {
    ...
    test('5. Offline login with wrong company is denied', () {
      ...
      final result = policy.evaluate(
        snapshot: snapshot,
        requestedUserId: 'user_a',
        requestedCompanyId: 'company_b', // Requesting Company B with Company A snapshot
        userStatus: 'active',
    ...
    test('8. Offline login with expired authorization (after 14 days) is rejected for Premium', () {
      final expiredTime = DateTime.now().toUtc().subtract(const Duration(days: 15));
      final snapshot = makeSnapshot(userId: 'user_a', companyId: 'company_a', lastVerified: expiredTime);
      final policy = OfflineLoginPolicy(expectedServerUrl: serverUrl, currentDeviceId: currentDevice);
    ...
    test('9. Offline login with device mismatch is handled by snapshot checks', () {
      ...
      expect(snapshot.matchesDevice(currentDevice), isFalse); // currentDevice = device_x
      expect(snapshot.matchesDevice('device_y'), isTrue);
    });

    test('10. Offline login with stale authorization version is rejected by version checks', () {
      final snapshot = makeSnapshot(
        userId: 'user_a',
        companyId: 'company_a',
    ...
    test('13. User B cannot inherit User A company context', () {
      final snapshotA = makeSnapshot(userId: 'user_a', companyId: 'company_a');
      final snapshotB = makeSnapshot(userId: 'user_b', companyId: 'company_b');
    ...
    test('14. Company A -> Company B switch invalidates permissions', () {
      ...
      final switched = session.copyWith(
        ...
        permissions: const {'accounting.view'}, // Switched company B permissions
      );
    ...
    test('17. Sync denied without entitlement', () {
      final entitlement = Entitlement.freeLocal('company_a'); // No sync capability
      final ctx = AuthorizationContext(
        userId: 'user_a',
        companyId: 'company_a',
    ...
    test('18. Sync denied when authorization is expired', () {
      ...
      final ctx = AuthorizationContext(
        ...
        entitlement: entitlement,
        authenticationMode: AuthenticationMode.sync,
        authorizationExpiresAt: DateTime.now().toUtc().subtract(const Duration(minutes: 1)), // Expired
      );
    ...
    test('19. Sync allowed when all authorization requirements are valid', () {
      final entitlement = Entitlement.premiumActive('company_a');
      final ctx = AuthorizationContext(
        userId: 'user_a',
        companyId: 'company_a',
        ...
        entitlement: entitlement,
        authenticationMode: AuthenticationMode.sync,
        authorizationExpiresAt: DateTime.now().toUtc().add(const Duration(hours: 1)),
      );
    ...
    test('20. Tenant isolation remains enforced at repository context level', () {
      const TenantContext contextA = TenantContext(companyId: 'company_a', userId: 'user_a');
      const TenantContext contextB = TenantContext(companyId: 'company_b', userId: 'user_a');

      expect(contextA.companyId, equals('company_a'));
      expect(contextB.companyId, equals('company_b'));
    });

    test('21. Riverpod authorization state reacts to provider invalidation', () {
      final container = ProviderContainer(
        overrides: [
  ...
  AuthControllerMock()
    ...
    state = AuthState(
      ...
      session: AuthSessionSnapshot(
        user: const AuthUser(id: 'mock_user', name: 'Mock', email: 'mock@nexabiz.com'),
        companies: const [AuthCompany(id: 'company_a', code: 'ca', name: 'Company A')],
        roles: const ['user'],
        permissions: const {'sales.view'},
        capturedAt: DateTime.now().toUtc(),
        currentCompanyId: 'company_a',
